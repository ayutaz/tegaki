...
| Phase | 概要 | Merge SHA |
|---|---|---|
| 1 | `packages/dataset-cjk-kanjivg` の雛形 | `5a560f6` |
| 2 | KanjiVG ローダー (`generator/src/dataset/kanjivg.ts`) | `3d8d0ed` |
| 3 | パイプライン統合 (`generator/src/dataset/pipeline.ts`, `--dataset kanjivg`) | `0a356aa` |
| 4 | `tegaki/fonts/ja-kana` 同梱 (180 chars) | `6b3098a` |
| 5 | Sigma-Lognormal リズム (`renderer/src/lib/rhythm.ts`) | `d2a9929` |
| 6 | 検証ツール (`renderer/src/lib/rhythm-metrics.ts`, `--strict`) | `37a46d2` |
| 7 | ドキュメント + サンプル | `9058e56` |
| 8 | リリース判断 / 成果物 | `c0335c4` |

> Phase 別の詳細チケット（成果物・Done 定義・設計思想・代替案比較）は完了後アーカイブしました。設計判断の記録が必要な場合は `git show e39fddb -- docs/tickets/` で復元可能。
