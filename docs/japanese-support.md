...
## 0. Implementation status (2026-04-19 時点)

> **設計から実装フェーズへ移行済み。以下の節は "実装当時の設計判断の記録" として保持する。**

| Phase | 概要 | Merge SHA | ticket |
|---|---|---|---|
| 1 | `packages/dataset-cjk-kanjivg` の雛形 | `5a560f6` | [phase1](./tickets/phase-1-dataset-package.md) |
| 2 | KanjiVG ローダー (`generator/src/dataset/kanjivg.ts`) | `3d8d0ed` | [phase2](./tickets/phase-2-kanjivg-loader.md) |
| 3 | パイプライン統合 (`generator/src/dataset/pipeline.ts`, `--dataset kanjivg`) | `0a356aa` | [phase3](./tickets/phase-3-pipeline-integration.md) |
| 4 | `tegaki/fonts/ja-kana` 同梱 (180 chars) | `6b3098a` | [phase4](./tickets/phase-4-kana-bundle.md) |
| 5 | Sigma-Lognormal リズム (`renderer/src/lib/rhythm.ts`) | `d2a9929` | [phase5](./tickets/phase-5-rhythm-synthesis.md) |
| 6 | 検証ツール (`renderer/src/lib/rhythm-metrics.ts`, `--strict`) | `37a46d2` | [phase6](./tickets/phase-6-validation.md) |
| 7 | ドキュメント + サンプル | `9058e56` | [phase7](./tickets/phase-7-docs-samples.md) |
| 8 | リリース判断 / 成果物 | `c0335c4` | [phase8](./tickets/phase-8-release.md) |

### 現状の到達点

- **仮名 bundle は同梱済み**: `import bundle from 'tegaki/fonts/ja-kana'` で即利用可能（ひらがな + カタカナ + 濁音/半濁音/小書き、計 180 chars、約 217 KB）。
- **漢字は generate-time の opt-in**: `TEGAKI_KANJIVG_FULL=1` を付けて `bun run fetch-kanjivg` → generate パイプラインを実行する形。ランタイム同梱はしない。
- **Sigma-Lognormal は runtime 計算**: bundle 無変更。CLI で `--rhythm lognormal|constant` を選択、renderer 側で `rhythm.ts` が動的に `t` を再マッピング。
- **validation ツール**: `rhythm-metrics.ts` で鐘型プロファイルの自己評価、CLI `--strict` でエラーとして検出。

### 設計時には未決で、実装段階で確定した項目

| 項目 | 実装時の決定 |
|---|---|
| JOYO / JINMEI allowlist | **空配列のまま出荷**。kana のみデフォルト、全 CJK Unified は `TEGAKI_KANJIVG_FULL=1` で opt-in。 |
| BUNDLE_VERSION | **bump せず**。rhythm は runtime 計算に寄せたため、既存 4 フォント bundle（Caveat / Italianno / Tangerine / Parisienne）は無変更で動作。 |
| TTF 戦略 (ja-kana) | Google Fonts `&text=` で subset した **97 KB の TTF のみ同梱**。full TTF (5 MB) は drop し、bundle 合計 217 KB に収めた。 |
| 評価方針 | 外部パネル MOS 評価は**前提外**。メンテナの自己評価（`rhythm-metrics` + 目視）で release 可と判断。 |

### 以降のセクションの位置付け

- §1〜§5、§7〜§9 は **設計判断の記録**として保持（代替案の考察も含む）。
- §6 の直後に [Implementation notes](#6-1-implementation-notes-実装時に設計から逸脱した点) を追加。設計からの逸脱点をまとめる。
...
### 6-1. Implementation notes — 実装時に設計から逸脱した点

ここは実装完了後（Phase 1〜8 マージ後）に追記した節。設計時の想定と実装コードの差分を記録する。

1. **allowlist の扱い**
   - 設計では JOYO / JINMEI を明示列挙する想定だったが、実装では **`JOYO` / `JINMEI` 配列は空のままコミット**。デフォルトでは kana (180 chars) のみを処理する。
   - 全 CJK Unified Ideographs を対象にしたい場合は **環境変数 `TEGAKI_KANJIVG_FULL=1`** を付けて実行（opt-in）。これによりカバレッジ管理を利用側に委ねる構造になった。
   - 配置: [packages/dataset-cjk-kanjivg/src/allowlist.ts](../packages/dataset-cjk-kanjivg/src/allowlist.ts)（予定）。

2. **xmldom で KanjiVG SVG をパースする際の DOCTYPE 処理**
   - KanjiVG の SVG は `xmlns:kvg` を **DOCTYPE 内の ATTLIST** で定義している。xmldom はこの形式を解釈できず属性が失われる。
   - 対策として [`normalizeKanjiSvg()`](../packages/generator/src/dataset/kanjivg.ts) で DOCTYPE ブロックを剥がしたうえで `xmlns:kvg` を `<svg>` 要素へ inline 注入してからパースする。これにより `kvg:element` / `kvg:type` 属性が通る。

3. **`fetch-kanjivg` の冪等性**
   - 設計時の擬似コードでは `rmSync(KANJIVG_DIR, { recursive: true })` を想定していたが、手動で編集した `README_KANJIVG.md` や `fix-overrides.json` まで消える。
   - 実装では **`.svg` ファイルのみ削除**する形に変更。README / 補助ファイルは保護される。

4. **Bun.fetch の Windows 不安定回避**
   - `bun run fetch-kanjivg` は `Bun.fetch()` で KanjiVG tarball を取得するが、Windows 環境で断続的に失敗するケースがあった。
   - `scripts/generate-ja-kana.ts` では **`Bun.spawnSync` 経由で curl を fallback** として呼び出す構造にした。

5. **ja-kana bundle の TTF 戦略**
   - Noto Sans JP の full TTF は 5 MB 超。pre-built bundle に同梱すると renderer パッケージのサイズが肥大化する。
   - 対策: Google Fonts `&text=<kana 180 chars>` でサブセットした **97 KB の TTF のみ** を同梱。合計 217 KB に収まった。
   - フル TTF が必要なケースは利用側で `fontsource` 等を使ってもらう前提。

6. **manifest.ts の取扱い**
   - `packages/dataset-cjk-kanjivg/src/manifest.ts` は **空プレースホルダーを git commit** する方針。実際に populated された manifest は `bun run fetch-kanjivg` 実行時に regenerate される。
   - 理由: KanjiVG の SVG 自体が `.gitignore` 対象（CC-BY-SA 3.0 ライセンス分離のため）。manifest の populated 版も git に入れない。

7. **`BUNDLE_VERSION` の据え置き**
   - 設計時は Phase 5 で bundle スキーマに rhythm データを埋め込む想定で、`BUNDLE_VERSION` の 0 → 1 bump を議論していた。
   - 実装では **rhythm を runtime 計算** (`renderer/src/lib/rhythm.ts`) に寄せたため、既存 4 フォント bundle (Caveat / Italianno / Tangerine / Parisienne) は **無変更で動作**。bump しないことで後方互換を自動維持。

8. **MOS 評価の位置付け**
   - 設計では「日本人評価者 5 段階平均 4.0 以上」を KPI に設定していたが、実装完了時点で **外部パネル MOS 評価は前提外**に変更。
   - 代わりに [rhythm-metrics.ts](../packages/renderer/src/lib/rhythm-metrics.ts) による自己評価（鐘型プロファイルの対称性・ピーク位置・終端減速率）+ メンテナ目視で release 可と判断する運用。
...
> この節は **設計当時の着手プラン** を記録したもの。実装は完了しているので、最新の実装配置は §0 [Implementation status](#0-implementation-status-2026-04-19-時点) を参照。

### Step 1: 筆順データ統合 → Shipped in `5a560f6` / `3d8d0ed` / `0a356aa`
...
2. `packages/generator/src/dataset/kanjivg.ts` に loader を追加
   ...
   - `kvg:StrokeNumber` でソート（※実装では `<path>` 出現順を採用）
...
### Step 2: リズム合成（Sigma-Lognormal） → Shipped in `d2a9929`

1. [packages/renderer/src/lib/rhythm.ts](../packages/renderer/src/lib/rhythm.ts) に `lognormalCDF / remapTime / strokeParams` を実装（runtime 計算）
2. `kvg:type` から終端種別を判定する `detectEndpointType()` を追加
3. renderer の `computeTimeline()` 経路で画ごとに σ/μ を算出し、t を非線形化
