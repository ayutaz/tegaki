...
/** Pluck every codepoint in the kana/CJK range from a string. */
...
export function PreviewApp() {
  ...
  useEffect(() => {
    ...
    setProcessing(true);
    let cancelled = false;
    // Preload the KanjiVG SVG for the selected char (if covered) before running
    // the pipeline — otherwise the dataset path silently falls back to the
    // heuristic skeletonizer on the first pass and stroke order comes out wrong.
    const run = async () => {
