...
export function extractTegakiBundle(input: ExtractBundleInput): TegakiBundleOutput {
  ...
  const glyphEntries: { char: string; totalAnimationDuration: number }[] = [];
  const glyphDataMap: Record<
    string,
    {
      advanceWidth: number;
      strokes: { points: { x: number; y: number; t: number; width: number }[]; delay: number; animationDuration: number }[];
    }
  > = {};

  for (const glyph of Object.values(output.glyphs)) {
    glyphEntries.push({ char: glyph.char, totalAnimationDuration: glyph.totalAnimationDuration });
...
function generateGlyphsModule(
  entries: { char: string; totalAnimationDuration: number }[],
  fontFileName: string,
  fontFamily: string,
  lineCap: LineCap,
  unitsPerEm: number,
  ...
  for (const { char, totalAnimationDuration } of entries) {
