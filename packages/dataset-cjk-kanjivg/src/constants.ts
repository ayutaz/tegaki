// Intentionally free of `node:*` imports so this module can be pulled into
// the browser (via the in-browser generator preview) without Vite's
// "Module node:path has been externalized" runtime error. Anything that
// needs to touch the filesystem lives in src/index.ts (Node) or
// scripts/fetch-kanjivg.ts (Node) and computes the SVG directory locally.
